...
public class BombThrower : LivingEntity, IEnemy, IDamageable {
 ...
 SpecificInit ();
  ...
  private bool BombWithinJumpRadius () {
    var cols = Physics2D.OverlapCircleAll (transform.position, bombDetectionRadius / 1.5f);
  ...
  private void AvoidBomb () {
    float x = PlayerCloserToLeftWall () ? -.5f : .5f;
    Jump (Vector2.up + Vector2.right * x);
  }
